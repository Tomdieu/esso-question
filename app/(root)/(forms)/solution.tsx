...
const SolutionScreen = (props: Props) => {
  ...
  const [formValues, setFormValues] = useState({
    solution: ideogram?.solutions[index].solution || "",
  });
  ...
  const handleSubmit = () => {
    try {
      ...
      IdeoGramm.updateSolution(ideogram, index, formValues);
      router.back();
    } catch (error) {
      if (error instanceof ZodError) {
        const formattedError = error.errors.reduce((acc: any, curr) => {
                    ...
                    >
                      <View
                        className={cn(
                          ...
                          {
                            "border-blue-600": formValues.solution === action,
                          }
                        )}
                      >
                      ...
                      <Text
                        ...
                        className={cn(
                          ...
                          {
                            "text-blue-600": formValues.solution === action,
                          }
                        )}
                      >
                        {action}
